...
using FHL.Hack.SmartAssistantBot.Dialogs.DayView;
using FHL.Hack.SmartAssistantBot.Dialogs.Escalate;
using FHL.Hack.SmartAssistantBot.Dialogs.Onboarding;
using FHL.Hack.SmartAssistantBot.Dialogs.Shared;
using FHL.Hack.SmartAssistantBot.Dialogs.Immigration;
using Luis;
using Microsoft.Bot.Builder;
using Microsoft.Bot.Builder.Dialogs;
using FHL.Hack.SmartAssistantBot.Dialogs.Tar;

namespace FHL.Hack.SmartAssistantBot.Dialogs.Main
{
    public class MainDialog : RouterDialog
    {
            ...
            : base(nameof(MainDialog))
        {
            ...
            AddDialog(new ImmigrationDialog(_services, _userState.CreateProperty<ImmigrationState>(nameof(ImmigrationState)), telemetryClient));
            AddDialog(new TarDialog(_services, _userState.CreateProperty<TarState>(nameof(TarState)), telemetryClient));
            AddDialog(new AboutBotDialog(_services, _conversationState, _userState, TelemetryClient));
            AddDialog(new AuthenticationDialog("connectionname"));
            AddDialog(new EscalateDialog(_services));
        }

        protected override async Task OnStartAsync(DialogContext dc,
        ...
        protected override async Task OnEventAsync(DialogContext dc,
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            if (dc.Context.Activity.Value != null)
            {
                ...
                if (value.action == "aboutEmpAssist")
                {
                    var view = new AboutBotResponses();
                    await view.ReplyWith(dc.Context, AboutBotResponses.ResponseIds.AboutBot);
                }

                //}
                if (value.action == "howsDayLooking")
                {
                    var view = new DayViewResponses();
                    await view.ReplyWith(dc.Context, DayViewResponses.ResponseIds.DayViewPrompt);
                }
            }
        }

        protected override async Task CompleteAsync(DialogContext dc,
            CancellationToken cancellationToken = default(CancellationToken))
        {
